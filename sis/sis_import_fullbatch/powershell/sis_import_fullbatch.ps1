...
$termMatchRegex = "^(Spring|Summer|Fall)\d{4}$" # Regex To validate the zip filenames match your Term SIS ID pattern. This example allows SIS IDs in the format of Spring. More info on RegEx in PowerShell: http://ss64.com/ps/syntax-regex.html
...
if($file_list.length -gt 0){
    foreach($file in $file_list){
        ...
        if($results -eq $null){
            write-host "   Unable to start an SIS import for this term. Please ensure a term with this SIS ID has already been created in Canvas. (SIS ID's are case sensitive) Also be sure the Canvas user has permission to import SIS data in Canvas."
            continue
        }
